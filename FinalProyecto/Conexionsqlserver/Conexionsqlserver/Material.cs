    ...
    public partial class Material : Form
    {
        ...
        private void LoadMaterialData()
        {
            ...
            try
            {
                ...
                adaptador.Fill(dt);
                dataGV_Material.DataSource = dt;
            }
            catch (Exception ex)
            {
                MessageBox.Show("Error al cargar los datos de material: " + ex.Message);
            }
            finally
            {
                conexion.cerrar();
        ...
        private void btn_buscar_Click(object sender, EventArgs e)
        {
            ...
            SELECT
                 ...
                 Material.Nombre
            FROM Material
            WHERE Material.Nombre LIKE @Busqueda";
        ...
        {
            if (dataGV_Material.SelectedRows.Count == 0)
            {
                MessageBox.Show("Seleccione el material para eliminar.");
                return;
            }


            int materialId = Convert.ToInt32(dataGV_Material.SelectedRows[0].Cells["Id"].Value);

            // Confirmar la eliminación
            DialogResult result = MessageBox.Show("¿Está seguro de eliminar este material?", "Confirmar Eliminación", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
